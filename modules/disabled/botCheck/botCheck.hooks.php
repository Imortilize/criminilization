<?php
	...
	if (!isset($_SESSION["actions"]))  $_SESSION["actions"] = 0;
	if (!isset($_SESSION["maxActions"])) $_SESSION["maxActions"] = mt_rand(50, 100);

	if ($_SESSION["actions"] >= $_SESSION["maxActions"]) {
		$_GET["page"] = "botCheck";
	} else {
		$_SESSION["lastAction"] = isset($_GET["page"])?$_GET["page"]:"";
	}

    new hook("userAction", function ($action) {
		$_SESSION["actions"] = $_SESSION["actions"] + 1;
	});
	...
	if( isset($_GET['_CAPTCHA']) ) {
	    ...
	    $captcha = imagescale($captcha, $bg_width);
	    ...
	    $box_width = 40; //abs($text_box_size[6] - $text_box_size[2]);
	    $box_height = 20; //abs($text_box_size[5] - $text_box_size[1]);
	    $text_pos_x_min = 0;
	    ...
	    $text_pos_y = mt_rand(0, $bg_height - $box_height);
	    ...
	    imagestring($captcha, 5, $text_pos_x, $text_pos_y, $captcha_config["code"], $color);
