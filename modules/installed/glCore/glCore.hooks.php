<?php
    ...
    new hook("adminWidget-html", function ($user) {
        ...
        return array(
            "sort" => 10000000,
            "size" => 12, 
            "html" => '<hr /><div class="text-center"><small>Criminilization v' . $page->modules["glCore"]["version"]."</small></div>",
            "type" => "html", 
            "title" => false
        );
    });
    ...
    new hook("adminWidget-alerts", function ($user) {
        ...
        if ($savedHash != $currentHash) return array( 
            "type" => "warning", 
            "text" => "The Criminilization core code has been altered, this can potentially break future upgrades!"
        );
        return false;
    });