<?php
    ...
    class kill extends module {
        ...
        $protection = $this->_settings->loadSetting("userProtection", true, 3);
				
				$signupTime = $user->getTimer('signup');
        $signupProtection = $signupTime + ($protection*24*60*60);
        if (time() < $signupProtection) {
            return $this->error($user->info->U_name." is under protection.");
        }
        $signupTime = $this->user->getTimer('signup');
        $signupProtection = $signupTime + ($protection*24*60*60);
        if (time() < $signupProtection) {
            return $this->error("You can not kill when you are under protection.");
        }
