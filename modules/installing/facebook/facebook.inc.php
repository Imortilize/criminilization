<?php
    ...
    class facebook extends module {
        ...
        public function signup($profile) {
        	...
        	$userExistCheck = $this->db->select("
        		SELECT * FROM users WHERE U_email = :email AND U_status IN (0, 1) ORDER BY U_status DESC LIMIT 0, 1
        	", array(
        		":email" => $profile["email"]
        	));

        	if ($userExistCheck && $userExistCheck["U_id"]) {
            	$this->db->insert("
                ...
                $_SESSION["userID"] = $userExistCheck["U_id"];
                header("Location:?");
                exit;
        	}
