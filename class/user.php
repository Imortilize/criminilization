<?php
    ...
    class user {
        ...
        public function bindVarsToTemplate() {
			...
			// Player stats
			$offence = 0;
			if (isset($this->info->US_offence)) {
				$offence = $this->info->US_offence;
			}
			
			$defence = 0;
			if (isset($this->info->US_defence)) {
				$defence = $this->info->US_defence;
			}
			
			$stealth = 0;
			if (isset($this->info->US_stealth)) {
				$stealth = $this->info->US_stealth;
			}
			
			$totalStats = ($offence + $defence + $stealth);
			$offencePerc = floor(($offence / $totalStats) * 100);
			$defencePerc = floor(($defence / $totalStats) * 100);
			$stealthPerc = floor(($stealth / $totalStats) * 100);
			
			// Make sure our calculated percentages add up to 100
			$totalPerc = ($offencePerc + $defencePerc + $stealthPerc);
			if ($totalPerc != 100) {
					
				// Grab the difference
				$difference = (100 - $totalPerc);

				// Create an array to easily detect the min/max of the stat percentages as needed
				$statArray = array(
					0 => $offence, 
					1 => $defence, 
					2 => $stealth
				...
				// The current stat percentages being awarded don't match the 100%
				// which means we have a math error due to ratios, so lets adjust
				// appropiately
				if ($totalPerc < 100) {
					// We have less stat percentage so find the stat percentage with the highest ratio and
					// add the remainder to it
					$index = array_search(max($statArray), $statArray);
				} else {
					// We have more stats so find the stat percentage with the lowest ratio and
					// remove the remainder from it
					$index = array_search(min($statArray), $statArray);
				}

				switch ($index) {
					case 0: { $offencePerc += $difference; } break;
					case 1: { $defencePerc += $difference; } break;
					case 2: { $stealthPerc += $difference; } break;
					default: break;
				}
			}
            ...
            if (isset($this->nextRank->R_name)) {
				$page->addToTemplate('nextRank', $this->nextRank->R_name);
			}
			
			$page->addToTemplate('off_perc', $offencePerc);
			$page->addToTemplate('def_perc', $defencePerc);
			$page->addToTemplate('stl_perc', $stealthPerc);
